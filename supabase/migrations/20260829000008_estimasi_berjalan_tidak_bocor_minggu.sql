-- ================================================================
-- MIGRATION: Estimasi "sedang dikerjakan" tidak bocor ke minggu lain
--
-- Bundle yang sedang dikerjakan (belum discan selesai) ditampilkan
-- sebagai estimasi upah di minggu tempat ia MULAI dikerjakan. Tapi
-- query-nya hanya mengecek batas ATAS (waktu_terima <= akhir periode)
-- tanpa batas BAWAH — sehingga selama bundle itu belum selesai, estimasi
-- yang sama terus "bocor" muncul di SETIAP minggu berikutnya yang dibuka,
-- bukan cuma di minggu tempat ia pertama kali mulai.
--
-- Ini sekarang dibatasi dua sisi: waktu_terima harus berada DI DALAM
-- periode yang sedang dilihat (p_dari..p_sampai), konsisten dengan aturan
-- yang sudah dipakai untuk upah yang benar-benar selesai (migrasi
-- upah_ikut_minggu_mulai_kerja) — keduanya sama-sama mengacu ke minggu
-- MULAI, bukan minggu saat ini.
--
-- Berlaku untuk overview_pekerja_periode, detail_pekerja_periode,
-- kroscek_pekerja_periode, kroscek_detail_pekerja, dan pelunasan
-- (lunaskan_upah_pekerja) — supaya pelunasan juga tidak diam-diam
-- membayar dimuka bundle dari minggu yang berbeda dari yang sedang dilihat.
-- ================================================================
...
-- Pelunasan (bayar dimuka) juga tidak boleh menjangkau bundle dari
-- minggu lain — hanya yang mulai dikerjakan di dalam periode yang
-- sedang dilihat.
