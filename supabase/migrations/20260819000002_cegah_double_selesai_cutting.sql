-- ================================================================
-- MIGRATION: Cegah double-selesai cutting
--
-- Bug: bundle yang statusnya sudah status_tahap.cutting.status = 'selesai'
-- masih bisa dipilih lagi di tab "Sedang Dipotong" (checkbox di detail
-- PO tidak membedakan bundle yang sudah selesai vs belum) lalu diproses
-- ulang lewat selesai_cutting_batch — akibatnya stok bahan/inventory
-- ikut terpotong DUA KALI untuk bundle yang sama, karena RPC ini tidak
-- ada penjagaan sama sekali terhadap bundle yang sudah selesai.
--
-- Fix utama ada di sisi tampilan (bundle yang sudah selesai difilter
-- keluar dari daftar yang bisa dipilih). Ini lapis pengaman tambahan
-- di database: tolak permintaan kalau ada bundle yang statusnya sudah
-- 'selesai' ikut terkirim.
-- ================================================================
...
DECLARE
    ...
    v_barcode_dup     text;
BEGIN
    -- Tolak kalau ada bundle yang statusnya sudah 'selesai' — cegah
    -- pemakaian bahan terpotong dua kali untuk bundle yang sama.
    SELECT b.barcode INTO v_barcode_dup
    FROM bundle b
    WHERE b.id IN (
