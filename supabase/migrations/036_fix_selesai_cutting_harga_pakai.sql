-- ================================================================
-- MIGRATION 036: Fix selesai_cutting_batch — set harga_pakai
--
-- Masalah: INSERT ke pemakaian_bahan tidak menyertakan harga_pakai,
-- sehingga kolom tersebut selalu 0 (default). Akibatnya laporan
-- total biaya bahan menjadi 0 atau salah.
--
-- Fix: Hitung harga_pakai = total_deduction × harga bahan.
--   Prioritas: harga_satuan dari inventory_batch (FIFO aktual)
--   Fallback:  harga_referensi dari inventory_item
-- ================================================================
...
DECLARE
    ...
    v_harga_per_unit    numeric;   -- harga per unit (dari batch atau referensi)
    v_harga_pakai_total numeric;   -- total biaya = qty × harga_per_unit
    -- FIFO variables
    v_batch             record;
    v_qty_remaining     numeric;
    v_qty_from_batch    numeric;
    v_first_batch_id    uuid;
    v_first_batch_harga numeric;   -- harga_satuan dari batch pertama yang dipakai
BEGIN
    ...
    LOOP
        ...
        SELECT id, nama, stok_aktual, harga_referensi INTO v_inv_item
        FROM inventory_item
        ...
        -- a. FIFO deduction dari inventory_batch, catat first_batch_id + harganya
        ...
        -- Fallback batch_id jika stok kurang
        ...
        -- b. Tentukan harga per unit:
        --    Prioritas: harga_satuan dari batch (FIFO aktual)
        --    Fallback:  harga_referensi dari inventory_item
        ...
        -- c. Update stok_aktual (boleh jadi negatif)
        UPDATE inventory_item
        SET stok_aktual = stok_aktual - v_total_deduction
        WHERE id = v_inv_item.id;

        -- d. Warning jika stok jadi negatif
        ...
        -- e. INSERT pemakaian_bahan dengan harga_pakai
        IF v_first_bundle_id IS NOT NULL THEN
            INSERT INTO pemakaian_bahan (
                ...
                v_po_item_id,
                v_inv_item.id,
                v_first_batch_id,        -- bisa NULL jika tidak ada batch sama sekali
                v_total_deduction,
                v_rate_per_pcs,
                v_harga_pakai_total,     -- total biaya = qty × harga (batch atau referensi)
                p_tenant_id,
                p_user_id
            )
